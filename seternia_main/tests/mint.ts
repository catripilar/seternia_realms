...
import { Faction, Class, Traits, MintData } from "./require"
...
describe("seternia", () => {
  ...
  it("should mint NFT successfully!", async () => {
    ...
    if(await con.getAccountInfo(MintTokenAccount)==null){
      const MintAssociatedToken = createAssociatedTokenAccountInstruction(
        wallet.publicKey,
        MintTokenAccount,
        wallet.publicKey,
        MintKey,
      ...
      transaction = transaction.add(MintAssociatedToken)
    }
    if(await con.getAccountInfo(TreasuryKey)!=null){
      ...
      const mint_data = await program.account.mintData.fetch(TraitKey) as MintData;
